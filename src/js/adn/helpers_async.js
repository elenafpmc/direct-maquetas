...
$(function() { // DOM Ready
	...
	/*	--------------------------------------------------
		...
		scrollTo: Modificacion de la función para retardar el scroll en los collapses
		cuando es necesario que hagan cosas antes de mostrarse 
	...
	$('[data-id-scroll-collpases]').on('click',function(){
			//ID del elemento donde queremos ir
			scrollToID = $(this).attr('data-id-scroll-collpases');
			...
			$(idToHide).one('webkitAnimationEnd oanimationend msAnimationEnd animationend',   
				function(e) {
					...
					if (idToShow) {
						...
						//La reseteamos para que pueda aparecer de nuevo
						$(idToHide).removeClass('animated').removeClass(hideAnimation);
	...
	if(windowWidth<smBreak){
		$('.content-block').css('height',windowHeight-150);
	}
